...
for run in "${runs[@]}"; do
  for size in "${sizes[@]}"; do
    for frac in "${frac_values[@]}"; do
      ...
      python llm-utils/quick_ft_llama.py --max_seq_len $max_seq_len --train_file "$train_file" --validation_file "$test_file" --output_dir "$output_dir" --config llm-utils/qlora.yaml >> zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/qa_$frac.log
      python llm-utils/predict.py --max_seq_len $max_seq_len --model_name_or_path "$output_dir" --model_kind causal-lm --input_file_path "$test_file" --output_file_path zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/qa_pred_$frac.csv --max_new_tokens $max_new_tokens --input_column input
      python compute_qa_metrics.py --input_file zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/qa_pred_$frac.csv >> zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/qa_$frac.log
      rm -rf $output_dir
    done
  done
done