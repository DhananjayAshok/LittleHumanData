...
for run in "${runs[@]}"; do
  for size in "${sizes[@]}"; do
    for frac in "${frac_values[@]}"; do
      ...
      python llm-utils/classification.py  --log_file "zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/clf_small_$frac.log" --train_file "$train_file" --validation_file "$test_file" --output_dir "$output_dir" $common >> "zoom_logs/$model/cot_$cot/$dataset_name/size_$size/run_$run/clf_small_$frac.out"
      rm -rf $output_dir
    done
  done
done